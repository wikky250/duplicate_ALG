...
int CInterCHeck::RealCheck(QString &result, CHECKPARAM *checkparam, int Wnd = -1,QString* pos)
{
	...
	try
	{
		...
		threshold(ho_ImageChannel[m_checkparam.i_Channel_Band], &ho_Region, m_checkparam.i_MinGray_Band, 255);
		...
		sub_image(ho_ImageChannel[m_checkparam.i_Channel1_NumPill], ho_ImageChannel[m_checkparam.i_Channel2_NumPill], &ho_ImageSub10, 1, 0);
		reduce_domain(ho_ImageSub10, ho_RegionBand, &ho_ImageReduced);
		threshold(ho_ImageReduced, &ho_Regions, m_checkparam.i_MinGray_NumPill, 255);
		...
		select_shape(ho_Circle8, &ho_SelectedRegions, "area", "and", m_checkparam.i_MinArea_NumPill, 99999);
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			smallest_rectangle2(ho_RegionPill, &hv_Row14, &hv_Column14, &hv_Phi4, &hv_Length14,
				&hv_Length24);
			gen_rectangle2(&ho_Rectangle8, hv_Row14, hv_Column14, hv_Phi4, hv_Length14 + m_checkparam.i_RectMove_Lack,
				hv_Length24 + m_checkparam.i_RectMove_Lack);
			sub_image(ho_ImageChannel[m_checkparam.i_Channel1_Lack], ho_ImageChannel[m_checkparam.i_Channel2_Lack], &ho_ImageSub13, 1, 0);
			threshold(ho_ImageSub13, &ho_Regions3, 0, m_checkparam.i_MaxGray_Lack);
			...
			select_shape(ho_ConnectedRegions34, &ho_SelectedRegions31, "area", "and", m_checkparam.i_MinArea1_Lack,
				99999);
			shape_trans(ho_SelectedRegions31, &ho_RegionTrans13, "convex");
			smallest_circle(ho_RegionTrans13, &hv_Row10, &hv_Column10, &hv_Radius5);
			gen_circle(&ho_Circle4, hv_Row10, hv_Column10, hv_Radius5 + m_checkparam.i_RadiusMove_Lack);
			...
			select_shape(ho_ConnectedRegions33, &ho_SelectedRegions29, (HTuple("area").Append("circularity")),
				"and", (HTuple(m_checkparam.i_MinArea2_Lack).Append(m_checkparam.d_Circularity_Lack)), (HTuple(99999).Append(1)));
		...
		intensity(ho_RegionPill, ho_ImageChannel[m_checkparam.i_Channel_Powder], &hv_Mean5, &hv_Deviation3);
		tuple_mean(hv_Mean5, &hv_Mean6);
		{
			...
			for (hv_Index5 = 1; hv_Index5.Continue(end_val104, step_val104); hv_Index5 += step_val104)
			{
				...
				intensity(ho_ObjectSelected4, ho_S, &hv_Mean4, &hv_Deviation);
				if (0 != (((hv_Mean4 - hv_Mean6).Abs()) > m_checkparam.i_DifMean_Powder))
				{
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			...
			if (0 != (hv_NumberPill != m_checkparam.i_Number_NumPill))
			{
		...
		smallest_rectangle2(ho_RegionUnion, &hv_Row, &hv_Column, &hv_Phi, &hv_Length1,
			&hv_Length2);
		gen_rectangle2(&ho_Rectangle, hv_Row, hv_Column, hv_Phi, hv_Length1 + m_checkparam.i_PlusLength1_Batch, hv_Length2 + m_checkparam.i_PlusLength2_Batch);
		...
		gray_dilation_rect(ho_ImageChannel[m_checkparam.i_Channel_Batch], &ho_ImageMax, 11, 11);
		sub_image(ho_ImageMax, ho_ImageChannel[m_checkparam.i_Channel_Batch], &ho_ImageSub1, 1, 0);
		threshold(ho_ImageSub1, &ho_Region4, m_checkparam.i_MinGray_Batch, 255);
		//dyn_threshold (ImageMax, G, RegionDynThresh, 10, 'light')
		opening_circle(ho_Region4, &ho_RegionOpening1, m_checkparam.d_Opening_Batch);
		intersection(ho_RegionOpening1, ho_SelectedRegions10, &ho_RegionIntersection);
		closing_circle(ho_RegionIntersection, &ho_RegionClosing, m_checkparam.d_Closing_Batch);
		...
		//***********ÅÝÕÖÒõÓ°ÇøÓò
		sub_image(ho_ImageChannel[m_checkparam.i_Channel2_Shadow], ho_ImageChannel[m_checkparam.i_Channel3_Shadow], &ho_ImageSub12, 1, 0);
		...
		gray_dilation_rect(ho_ImageChannel[m_checkparam.i_Channel1_Shadow], &ho_ImageMax8, 31, 31);
		sub_image(ho_ImageMax8, ho_ImageChannel[m_checkparam.i_Channel1_Shadow], &ho_ImageSub15, 1, 0);
		threshold(ho_ImageSub15, &ho_Regions5, m_checkparam.i_MinGray_Shadow, 255);
		...
		opening_circle(ho_RegionIntersection25, &ho_RegionOpening27, m_checkparam.d_Opening_Shadow);
		...
		select_shape(ho_ConnectedRegions41, &ho_SelectedRegions37, ((HTuple("width").Append("height")).Append("area")),
			"and", ((HTuple(m_checkparam.i_Width_Shadow).Append(m_checkparam.i_Length_Shadow)).Append(m_checkparam.i_Area_Shadow)), ((HTuple(99999).Append(99999)).Append(99999)));
		...
		gen_circle(&ho_Circle7, hv_Row26, hv_Column26, (hv_Radius8 / hv_Radius8) * m_checkparam.i_Radius_Shadow);
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			reduce_domain(ho_ImageChannel[m_checkparam.i_Channel_Crack], ho_RegionDifference24, &ho_ImageReduced18);
			...
			select_shape_xld(ho_UnionContours, &ho_SelectedXLD, (HTuple("contlength").Append("circularity")),
				"and", (HTuple(m_checkparam.i_Length_Crack).Append(0)), (HTuple(99999).Append(m_checkparam.d_Circularity_Lack)));
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			...
			threshold(ho_ImageSub12, &ho_Region29, 0, m_checkparam.i_MaxGray_Dirty);
			...
			select_shape(ho_ConnectedRegions39, &ho_SelectedRegions35, (HTuple("area").Append("circularity")),
				"and", (HTuple(m_checkparam.i_MinArea_BlackSpot).Append(m_checkparam.d_Circularity_BlackSpot)), (HTuple(99999).Append(1)));
			...
			if (0 != (hv_Area8 > m_checkparam.i_MinArea_Dirty))
			{
		...
		{
			gray_dilation_rect(ho_ImageChannel[m_checkparam.i_Channel_Fissure], &ho_ImageMax9, 11, 11);
			sub_image(ho_ImageMax9, ho_ImageChannel[m_checkparam.i_Channel_Fissure], &ho_ImageSub20, 1, 0);
			threshold(ho_ImageSub20, &ho_Region33, m_checkparam.i_MinGray_Fissure, 255);
			threshold(ho_ImageChannel[m_checkparam.i_Channel_Fissure], &ho_Region21, 0, 155);
			...
			select_shape(ho_ConnectedRegions29, &ho_SelectedRegions23, (HTuple("area").Append("circularity")),
				"and", (HTuple(m_checkparam.i_MinArea_Fissure).Append(0)), (HTuple(99999).Append(m_checkparam.d_Circularity_Fissure)));
		...
		gray_erosion_rect(ho_ImageChannel[m_checkparam.i_Channel1_Iner], &ho_ImageMax6, 7, 7);
		sub_image(ho_ImageChannel[m_checkparam.i_Channel1_Iner], ho_ImageChannel[m_checkparam.i_Channel2_Iner], &ho_ImageSub9, 1, 0);
		threshold(ho_ImageSub9, &ho_Region25, m_checkparam.i_MinGray_Iner, 255);
		closing_circle(ho_Region25, &ho_RegionClosing12, m_checkparam.d_Closing_Iner);
		opening_circle(ho_RegionClosing12, &ho_RegionOpening22, m_checkparam.d_Opening_Iner);
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			...
			count_obj(ho_SelectedRegions, &hv_Number13);
			{
				...
				for (hv_Index4 = 1; hv_Index4.Continue(end_val270, step_val270); hv_Index4 += step_val270)
				{
					...
					if (0 != (((hv_Area5 - hv_Mean1).Abs()) > m_checkparam.i_MinArea_Half))
					{
			...
			if (0 != (hv_Number14 > 0))
			{
				...
				gen_rectangle1(&ho_Rectangle10, hv_Row16 +m_checkparam.i_RectMove1_Half, hv_Column16 +m_checkparam.i_RectMove1_Half, hv_Row25 + m_checkparam.i_RectMove2_Half,
					hv_Column25 + m_checkparam.i_RectMove2_Half);
				set_draw(Wnd == -1 ? m_ShowLabel[0] : Wnd, "margin");
				set_color(Wnd == -1 ? m_ShowLabel[0] : Wnd, "red");
				disp_obj(m_hoLiveImage, Wnd == -1 ? m_ShowLabel[0] : Wnd);
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			erosion_circle(ho_RegionTrans14, &ho_RegionErosion11, m_checkparam.d_Erosion_Break);
			union1(ho_RegionErosion11, &ho_RegionUnion15);
			reduce_domain(ho_ImageChannel[m_checkparam.i_Channel_Break], ho_RegionUnion15, &ho_ImageReduced19);
			...
			select_shape_xld(ho_UnionContours1, &ho_SelectedXLD1, (HTuple("contlength").Append("circularity")),
				"and", (HTuple(m_checkparam.i_Length_Break).Append(0)), (HTuple(1000).Append(m_checkparam.d_Circularity_Break)));
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			select_shape(ho_ConnectedRegions32, &ho_SelectedRegions28, (HTuple("area").Append("circularity")),
				"and", (HTuple(m_checkparam.i_MinArea_PillDirty).Append(m_checkparam.d_Circularity_PillDirty)), (HTuple(99999).Append(1)));
		...
		sub_image(ho_ImageChannel[m_checkparam.i_Channel1_PressedBubble], ho_ImageChannel[m_checkparam.i_Channel2_PressedBubble], &ho_ImageSub17, 1, 0);
		erosion_circle(ho_RegionPill, &ho_RegionErosion10, 10.5);


		threshold(ho_ImageSub17, &ho_Region27, 0, 20);
		intersection(ho_Region27, ho_RegionErosion10, &ho_RegionIntersection26);
		opening_circle(ho_RegionIntersection26, &ho_RegionOpening32, 2.5);
		connection(ho_RegionOpening32, &ho_ConnectedRegions43);
		select_shape(ho_ConnectedRegions43, &ho_SelectedRegions38, "area", "and", 50,
			99999);
		...
		sub_image(ho_ImageChannel[m_checkparam.i_Channel1_PressedBubble], ho_ImageChannel[m_checkparam.i_Channel3_PressedBubble], &ho_ImageSub18, 1, 0);
		gray_erosion_rect(ho_ImageSub18, &ho_ImageMin1, 11, 11);
		sub_image(ho_ImageSub18, ho_ImageMin1, &ho_ImageSub19, 1, 0);
		threshold(ho_ImageSub19, &ho_Region32, 90, 255);
		intersection(ho_Region32, ho_RegionErosion10, &ho_RegionIntersection27);
		fill_up(ho_RegionIntersection27, &ho_RegionFillUp17);
		opening_circle(ho_RegionFillUp17, &ho_RegionOpening33, 1.5);
		closing_circle(ho_RegionOpening33, &ho_RegionClosing16, 5.5);
		connection(ho_RegionClosing16, &ho_ConnectedRegions44);
		select_shape(ho_ConnectedRegions44, &ho_SelectedRegions39, "area", "and", 30,
			99999);
		...
		//***********ÒõÓ°ÇøÓòÈ±ÏÝ
		sub_image(ho_ImageChannel[m_checkparam.i_Channel1_ShadowDefect], ho_ImageChannel[m_checkparam.i_Channel2_ShadowDefect], &ho_ImageSub16, 1, 0);
		...
		select_shape(ho_ConnectedRegions40, &ho_SelectedRegions36, "area", "and", 30,
			99999);
		...
		sub_image(ho_ImageChannel[m_checkparam.i_Channel1_Folds], ho_ImageChannel[m_checkparam.i_Channel2_Folds], &ho_ImageSub14, 1, 0);
		...
		if (0 != m_checkparam.b_CheckPill)
		{
			...
			threshold(ho_ImageChannel[m_checkparam.i_Channel_Unclear], &ho_Region4, 175, 255);
