...
int CInterCHeck::RealCheck(QString &result, CHECKPARAM *checkparam, int Wnd = -1)
{
	...
	try
	{
		...
		if (0 != (hv_Area8 > 50))
		{
			...
			set_tposition(Wnd == -1 ? m_ShowLabel[0] : Wnd, 10, 10);
			result = QString::fromLocal8Bit("片剂区域缺陷");
			write_string(Wnd == -1 ? m_ShowLabel[0] : Wnd, "片剂区域缺陷");
			return 1;
		}
		...
		if (0 != (hv_Number2 != 0))
		{
			...
			set_tposition(Wnd == -1 ? m_ShowLabel[0] : Wnd, 10, 10);
			result = QString::fromLocal8Bit("片剂内部缺陷");
			write_string(Wnd == -1 ? m_ShowLabel[0] : Wnd, "片剂内部缺陷");
			return 1;
			// stop(); only in hdevelop
			//continue
		}
