...
bool CInterCHeck::RealCheck(QString &result, CHECKPARAM *checkparam, int Wnd = -1)
{
	...
	try
	{
		...
		// Local iconic variables 
		Hobject  ho_Image, ho_EmptyRegion_Out, ho_EmptyRegion_Top;
		...
		add_image(ho_Image2, ho_Image3, &ho_ImageResult, 0.5, 0);
		...
		threshold(ho_ImageResult3, &ho_Region, hv_MiddleThread, 255);
		...
		for (hv_Co_Index = 1; hv_Co_Index <= 6; hv_Co_Index += 1)
		{
			...
			hv_R_s = 330;
			hv_R_sEnd = 700;
			gen_rectangle1(&ho_Rectangle, hv_R_s, hv_C_s, hv_R_sEnd, ((hv_Width / 6)*hv_Co_Index) + 20);
			...
			if (0 != (HTuple(hv_Height_CAP < 160).And(hv_Height_CAP > 170)))
			{
				concat_obj(ho_EmptyRegion_Top, ho_SelectedRegions4, &ho_EmptyRegion_Top);
				hv_Tuple_Error = hv_Tuple_Error.Concat(hv_Co_Index);
				qslre[5 - (hv_Co_Index[0].I() - 1)] = QString::fromLocal8Bit("Length");
				continue;
			}

			intersection(ho_SelectedRegions4, ho_Region_1st, &ho_RegionIntersection11);
			opening_circle(ho_RegionIntersection11, &ho_RegionIntersection11, 3.5);
			...
			opening_circle(ho_RegionIntersection12, &ho_RegionIntersection12, 3.5);
			...
			hv_h = hv_Height_CAP / 2;
			if (0 != (HTuple(HTuple(HTuple(HTuple(hv_H1 > ((hv_Height_CAP / 2) - 10)).And(hv_H1 < ((hv_Height_CAP / 2) + 10))).And(hv_H2 > ((hv_Height_CAP / 2) - 10))).And(hv_H2 < ((hv_Height_CAP / 2) + 10))).Not()))
			{
				union2(ho_EmptyRegion_Inner, ho_RegionOpening11, &ho_EmptyRegion_Inner);
				hv_Tuple_Error = hv_Tuple_Error.Concat(hv_Co_Index);
				qslre[5 - (hv_Co_Index[0].I() - 1)] = QString::fromLocal8Bit("ColorError");
				continue;
			}
			...
			smallest_rectangle1(ho_RegionTrans2, &hv_Row11, &hv_Column11, &hv_Row21, &hv_Column21);
			gen_rectangle1(&ho_Rectangle, (((hv_Row21 - hv_Row11) / 5) * 2) + hv_Row11, hv_Column11,
				hv_Row21, hv_Column21);

			intersection(ho_RegionTrans2, ho_Rectangle, &ho_RegionIntersection);
			erosion_circle(ho_RegionIntersection, &ho_RegionErosion, 5.5);
			union2(ho_EmptyRegion_Out, ho_RegionErosion, &ho_EmptyRegion_Out);
			reduce_domain(ho_Image3, ho_RegionErosion, &ho_ImageReduced);
			gray_dilation_rect(ho_ImageReduced, &ho_ImageMax, 11, 11);
			sub_image(ho_ImageMax, ho_ImageReduced, &ho_ImageSub, 1, 0);
			intensity(ho_RegionErosion, ho_ImageSub, &hv_Mean, &hv_Deviation);
			min_max_gray(ho_RegionErosion, ho_ImageSub, 0, _, &hv_Max1, _);
			if (0 != (HTuple(hv_Deviation > 15).Or(hv_Max1 > 90)))
			{
				concat_obj(ho_EmptyRegion_Intensity, ho_RegionErosion, &ho_EmptyRegion_Intensity
				);
				hv_Tuple_Error = hv_Tuple_Error.Concat(hv_Co_Index);
				qslre[5 - (hv_Co_Index[0].I() - 1)] = QString::fromLocal8Bit("sunken");
				continue;
			}
			erosion_circle(ho_RegionIntersection, &ho_RegionErosion3, 3.5);
			...
			hv_R_s = hv_Height - hv_DOWNBoundary;
			hv_R_sEnd = hv_Height;
			...
			select_shape_std(ho_ConnectedRegions4, &ho_SelectedRegions5, "max_area", 70);
			opening_circle(ho_SelectedRegions5, &ho_RegionOpening9, 10.5);
			shape_trans(ho_RegionOpening9, &ho_RegionTrans3, "convex");
			smallest_rectangle1(ho_RegionTrans3, &hv_Row12, &hv_Column12, &hv_Row22, &hv_Column22);
			gen_rectangle1(&ho_Rectangle1, hv_Row12, hv_Column12, (((hv_Row22 - hv_Row12) / 5) * 3) + hv_Row12,
				hv_Column22);

			intersection(ho_RegionTrans3, ho_Rectangle1, &ho_RegionIntersection8);
			erosion_circle(ho_RegionIntersection8, &ho_RegionErosion5, 5.5);
			union2(ho_EmptyRegion_Out, ho_RegionErosion5, &ho_EmptyRegion_Out);
			reduce_domain(ho_Image3, ho_RegionErosion5, &ho_ImageReduced2);
			gray_dilation_rect(ho_ImageReduced2, &ho_ImageMax3, 11, 11);
			sub_image(ho_ImageMax3, ho_ImageReduced2, &ho_ImageSub3, 1, 0);
			intensity(ho_RegionErosion5, ho_ImageSub3, &hv_Mean2, &hv_Deviation2);
			min_max_gray(ho_RegionErosion5, ho_ImageSub3, 0, _, &hv_Max, _);
			if (0 != (HTuple(hv_Deviation > 15).Or(hv_Max > 90)))
			{
				concat_obj(ho_EmptyRegion_Intensity, ho_RegionErosion5, &ho_EmptyRegion_Intensity
				);
				hv_Tuple_Error = hv_Tuple_Error.Concat(hv_Co_Index);
				qslre[5 - (hv_Co_Index[0].I() - 1)] = QString::fromLocal8Bit("sunken");
				continue;
